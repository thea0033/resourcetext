    ...
    pub fn select_components(&mut self, config: &mut Config, max_amounts: Option<Vec<usize>>) -> Option<(ComponentID, usize)> {
        ...
        if let Some(val) = max_amounts {
            ...
            let amount: usize = config.buffer.get_valid_flush(
                &format!("How many components do you want to select? (between 0 and {})", val[id.id()]),
            ...
            let amount = config
                .buffer
                .get_flush("How many components do you want to select?", "Please enter a valid number. ");
            Some((id, amount))
        }
    }
    /// Gets a component from the ui.
    pub fn select_component(&mut self, config: &mut Config) -> Option<ComponentID> {
        let numbered = self.cmp.display();
        let options: OptionTable = OptionTable::new("Select a component".to_string(), numbered, config.context.grab(constants::SELECT));
        // TODO: Re-implement pasting
